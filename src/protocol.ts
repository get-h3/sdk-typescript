 ...
 *
 * AUTO-GENERATED from get-h3/protocol/schemas/v1/*.json.
...
export const AttachmentSchema = z.object({
    type: z.enum(["image", "file", "audio", "video"] as const),
    url: z.string(),
    mime_type: z.string(),
  });
...
export const SessionStateSchema = z.object({
    ...
    total_tool_calls: z.number().int().min(0).default(0),
    total_llm_calls: z.number().int().min(0).default(0),
    cost_so_far: z.number().min(0).default(0),
    started_at: z.string().optional(),
  });
export type SessionState = z.infer<typeof SessionStateSchema>;

export const ConfigSchema = z.object({
    max_iterations: z.number().int().min(1).default(100),
    timeout_seconds: z.number().int().min(1).default(60),
    project_dir: z.string().optional(),
    max_tool_calls_per_turn: z.number().int().min(0).optional(),
...
export const WaitSchema = z.object({
    ...
    poll_endpoint: z.string().optional(),
  });
...
export const EndSchema = z.object({
    reason: z.enum(["task_complete", "user_requested", "error", "timeout", "rate_limited", "cancelled"] as const),
...
export const CancelRequestSchema = z.object({
    session_id: z.string(),
    reason: z.enum(["user_interrupt", "timeout", "system"] as const),
  });
...
export const HealthResponseSchema = z.object({
    status: z.enum(["ok", "degraded", "down"] as const),
    version: z.string(),
    ...
    capabilities: z.array(z.enum(["tool_call", "llm_call", "text", "wait", "delegate", "end"] as const)).optional(),
...
export const SessionResponseSchema = z.object({
    ...
    turn_count: z.number().int().min(0).default(0),
    status: z.enum(["active", "completed", "expired", "cancelled"] as const),
    current_decision: z.string().optional(),
    current_decision_type: z.enum(["tool_call", "llm_call", "text", "wait", "delegate", "end"] as const).optional(),
  });
