...
const FILE_NAMES: Record<
  string,
  { name: string; enums?: Record<string, string> }
> = {
  ...
  "result-request": {
    name: "ResultRequest",
    enums: { "result.type": "ResultType" },
  },
  "cancel-request": {
    name: "CancelRequest",
    enums: { reason: "CancelReason" },
  },
  "health-response": {
    name: "HealthResponse",
    enums: { status: "HealthStatus", capabilities: "Capability" },
  },
  "error-response": {
    name: "ErrorResponse",
    enums: { "error.code": "ErrorCode" },
  },
  "session-response": {
    name: "SessionResponse",
    enums: { status: "SessionStatus" },
  },
};
...
const EXTRA_TYPES: Record<
  string,
  { schema: JsonSchema; enums?: Record<string, string> }
> = {
  ResultPayload: {
    schema: {
      type: "object",
...
const INLINE_ENUM_MAX = 50; // always use single-line enum format
...
class Resolver {
  ...
  resolveRef(ref: string): { schema: JsonSchema; name: string } | null {
    if (ref.startsWith("#/definitions/")) {
      ...
      if (this.enums.has(name)) {
        return {
          schema: { type: "string", enum: this.enums.get(name)! },
          name,
        };
      }
      return null;
  ...
  zodExpr(schema: JsonSchema, level: number, fieldName?: string): string {
    // Check for REPLACE override first — these must fire before enum/$ref/const
    // early exits, since overrides intentionally replace enum-typed fields
    // with looser types (e.g., role enum → z.string()).
    let override: string | undefined;
    if (fieldName && this.currentDefName) {
      override = FIELD_OVERRIDES[this.currentDefName]?.[fieldName];
    }
    if (override?.startsWith("REPLACE:")) {
      return override.slice("REPLACE:".length);
    }

    // Early exits — these don't get field-level overrides
    ...
    switch (schema.type) {
      ...
      case "array": {
        if (schema.items) {
          expr = `z.array(${this.zodExpr(schema.items, level)})`;
        } else {
          expr = "z.array(z.any())";
        }
        break;
      ...
      case "object": {
        if (!schema.properties) {
          ...
          for (const [key, prop] of Object.entries(schema.properties)) {
            let propExpr = this.zodExpr(prop, level + 1, key);
            if (!required.has(key)) {
              if (!propExpr.includes(".default(")) {
                propExpr = `${propExpr}.optional()`;
              }
            }
            props.push(`${indent(level + 1)}${key}: ${propExpr},`);
          }
          expr = `z.object({\n${props.join("\n")}\n${indent(level)}})`;
        }
        break;
      ...
      default:
        if (schema.oneOf) expr = "z.any() /* oneOf */";
        else expr = "z.any()";
        break;
    }

    // Apply suffix override
    if (override) {
      expr = expr + override;
    }
...
function generate(files: SchemaFile[], protocolDir: string): string {
    ...
    if (defSchema.type === "object" && defSchema.properties) {
      for (const [propKey, propSchema] of Object.entries(
        ...
        if (propSchema.enum && !resolver.enums.has(propKey)) {
          const enumName =
            defName + propKey.charAt(0).toUpperCase() + propKey.slice(1);
  ...
  lines.push(
    ` * DO NOT EDIT BY HAND — run: npx tsx scripts/generate-schemas.ts`,
  );
  lines.push(` */`);
  lines.push(``);
  ...
  for (const defName of defOrder) {
    ...
    if (!schema) continue;
    resolver.currentDefName = exportName;
  ...
  if (decisionFile) {
    resolver.currentDefName = "Decision";
...
function main() {
  ...
  if (fs.existsSync(outPath)) {
    ...
    if (existing === output) {
      changed = false;
      console.error(
        "No changes — generated output matches current protocol.ts",
      );
    }
  }
